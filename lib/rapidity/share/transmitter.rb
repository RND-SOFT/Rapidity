module Rapidity
  module Share
    class Transmitter < Base
      ...
      def release_queue(limit_or_str, count: 1)
        name = limit_or_str.is_a?(Limit) ? limit_or_str.name : limit_or_str
        response = wrap_executed_script do
          @pool.with do |conn|
            conn.with do |r|
              r.evalsha(@lua_release_queue, keys: [name], argv: [count])
            end
          end
        end
      ...
      def available_in(list_limits_or_str, tokens: 1)
        limits = if list_limits_or_str[0].is_a?(Limit)
          list_limits_or_str.map {|it| it.name}
        else
          list_limits_or_str
        ...
        response = wrap_executed_script do
          @pool.with do |conn|
            conn.with do |r|
              r.evalsha(@lua_available_in, keys: [*limits], argv: [tokens])
            end
          end
        end
